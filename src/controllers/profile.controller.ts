...
export class ProfileController {
  ...
  private formatProfileData(profile: ProfileDocument) {
    try {
      ...
      if (basicSection) {
        const fullNameField = basicSection.fields?.find(
          (f) => f.key === "fullName"
        );
        if (fullNameField) {
      ...
      if (formattedData.name === "Untitled Profile") {
        ...
        logger.warn(
          `Fixed "Untitled Profile" in final output to: ${formattedData.name}`
        );
      }
      ...
      try {
        logger.error(
          `Profile data that caused error: ${JSON.stringify({
            id: profile._id,
      ...
      try {
        if (profile.profileInformation?.username) {
          ...
          logger.info(
            `Using username "${fallbackName}" for fallback profile name`
          );
        }
      } catch (nameError) {
  ...
  createProfile = asyncHandler(async (req: Request, res: Response) => {
    ...
    const { templateId, profileInformation, sections, profileLocation } =
  ...
  setEnabledFields = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!Array.isArray(toggles))
      throw createHttpError(400, "Expected array of field toggles");

    const updated = await this.service.setEnabledFields(
      profileId,
      userId,
      toggles
    );
  ...
  updateProfileContent = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!Array.isArray(updates))
      throw createHttpError(400, "Expected array of field updates");

    const updated = await this.service.updateProfileContent(
      profileId,
      userId,
      updates
    );
  ...
  getUserProfiles = asyncHandler(async (req: Request, res: Response) => {
    ...
    const formattedProfiles = profiles.map((profile) =>
      this.formatProfileData(profile)
    );
  ...
  deleteProfile = asyncHandler(async (req: Request, res: Response) => {
    ...
    logger.info(
      `Deleting profile ${profileId} by user ${userId}${deleteUserAccount ? " with user account deletion" : ""}`
    );
    ...
    logger.info(
      `User object: ${JSON.stringify({
        ...
        _doc: (req.user as any)?._doc
          ? { role: (req.user as any)?._doc?.role }
          : "no _doc",
      })}`
    );
    ...
    if (deleteUserAccount && deleted) {
      ...
      logger.info(
        `Role check for deleteUserAccount: userRole=${userRole}, isAdminHeader=${isAdminHeader}, isAdminCookie=${isAdminCookie}`
      );
      logger.info(
        `User object: ${JSON.stringify({
          id: user._id,
      ...
      const isAdmin =
        ["admin", "superadmin"].includes(userRole as string) ||
        isAdminHeader ||
        isAdminCookie;
      ...
      if (!isAdmin && !forceAllowDeletion) {
        logger.warn(
          `User ${user._id} attempted to delete user account but has role: ${userRole}`
        );
        throw createHttpError(403, "Only admins can delete user accounts");
      } else if (!isAdmin && forceAllowDeletion) {
        logger.warn(
          `User ${user._id} is not an admin but deletion is being forced for testing`
        );
      }
      ...
      try {
        ...
        if (creatorId) {
          logger.info(
            `Admin ${user._id} is deleting user account ${creatorId} along with profile ${profileId}`
          );
          ...
          logger.info(
            `User account ${creatorId} deleted along with profile ${profileId}`
          );
        } else {
        ...
        logger.error(
          `Failed to delete user account for profile ${profileId}:`,
          error
        );
        logger.error(
  ...
  /** POST /default */
  createDefaultProfile = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      if (existingPersonalProfile) {
        logger.info(
          `User ${userId} already has a personal profile: ${existingPersonalProfile._id}. Returning existing profile.`
        );

        // Format the existing profile data for frontend consumption
        const formattedProfile = this.formatProfileData(
  ...
  getAllProfiles = asyncHandler(async (req: Request, res: Response) => {
    ...
    logger.info(
      `Admin user ${user._id} (${user.email}) requesting all profiles`
    );

    // Parse query parameters for pagination and filtering
    let limit = parseInt(req.query.limit as string) || 20; // Default limit to 20 if not provided
   ...
   */
  setAvailability = asyncHandler(
    async (req: Request, res: Response, next: NextFunction): Promise<void> => {
      const { profileId } = req.params;
      const availabilityData = req.body;

      const availability = await this.service.setAvailability(
        profileId,
        availabilityData
      );
  ...
  updateAvailability = asyncHandler(
    async (req: Request, res: Response, next: NextFunction): Promise<void> => {
      ...
      const availability = await this.service.updateAvailability(
        profileId,
        updates
      );
  ...
  async getCommunityProfiles(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const profiles = await this.service.getCommunityProfiles(
        filters,
        skip,
        limit
      );
   ...
   */
  deleteDuplicatePersonalProfiles = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      logger.info(
        `Admin ${user._id} initiated duplicate personal profile deletion`
      );
      ...
      try {
        ...
        throw createHttpError(
          500,
          "Failed to delete duplicate personal profiles"
        );
      }
    }
  );
